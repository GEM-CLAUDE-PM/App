...
export default function HSEWorkspace({ project: selectedProject, projectId: projectIdProp, ctx: ctxProp }: HSEProps) {
  ...
  // localStorage keys
  const HSE_KEYS = {
    INCIDENTS:   'gem_hse_incidents',
    TRAININGS:   'gem_hse_trainings',
    INSPECTIONS: 'gem_hse_inspections',
    VIOLATIONS:  'gem_hse_violations',
  };
  ...
  // ── Local state ────────────────────────────────────────────────
  const loadHSE = <T,>(key:string, seed:T[]):T[] => {
  ...
  const [workerCerts, setWorkerCerts]   = React.useState<WorkerCert[]>(() => {
    try { const r = localStorage.getItem(`gem_hse_worker_certs_${project?.id||''}`); return r ? JSON.parse(r) : SEED_CERTS; } catch { return SEED_CERTS; }
  });
  ...
  const [incidents, setIncidents]         = React.useState<Incident[]>(() => loadHSE(HSE_KEYS.INCIDENTS, SEED_INCIDENTS));
  const [trainings, setTrainings]         = React.useState<Training[]>(() => loadHSE(HSE_KEYS.TRAININGS, SEED_TRAININGS));
  const [violations, setViolations]       = React.useState<Violation[]>(() => loadHSE(HSE_KEYS.VIOLATIONS, SEED_VIOLATIONS));
  const [inspections, setInspections]     = React.useState<Inspection[]>(() => loadHSE(HSE_KEYS.INSPECTIONS, SEED_INSPECTIONS));
            ...
            <div className="bg-rose-50 border border-rose-200 rounded-2xl p-4 space-y-3 animate-in slide-in-from-top-2 duration-200">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button onClick={() => {
                  ...
                  setIncidents(updated); save(HSE_KEYS.INCIDENTS, updated);
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 space-y-3 animate-in slide-in-from-top-2 duration-200">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button onClick={() => {
                  ...
                  setViolations(updated); save(HSE_KEYS.VIOLATIONS, updated);
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 space-y-3 animate-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <button onClick={() => {
                      ...
                      setTrainings(updated); save(HSE_KEYS.TRAININGS, updated);
                ...
                <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 space-y-3 animate-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <button onClick={() => {
                      ...
                      localStorage.setItem(`gem_hse_worker_certs_${project?.id||''}`, JSON.stringify(updated));
