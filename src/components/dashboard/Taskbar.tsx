...
import {
  LayoutDashboard, FolderKanban, CalendarDays, Users,
  FileText, ClipboardList, ClipboardCheck, Sparkles,
  UploadCloud, UserPlus, ShieldCheck, Printer,
  Plus, Package, AlertTriangle, ChevronUp, ChevronDown,
  CalendarPlus, UserPlus2, Banknote, Building2,
  ListChecks, ShieldAlert, TrendingUp, Wrench,
...
function Tooltip({ label, children }: { label: string; children: React.ReactNode }) {
  ...
  return (
    <div className="relative flex flex-col items-center"
      ...
      {children}
      {visible && (
        <div className="absolute bottom-full mb-2 px-2.5 py-1 bg-slate-800 text-white text-[11px]
...
// ── DockButton ────────────────────────────────────────────────────────────────
function DockButton({
  icon: Icon, label, onClick, active, colorKey, badge,
}: {
  icon: any; label: string; onClick: () => void;
  active?: boolean; colorKey?: string; badge?: number;
}) {
  return (
    <Tooltip label={label}>
      <button onClick={onClick}
        className={`relative w-10 h-10 rounded-xl flex items-center justify-center
          ...
          ${active && colorKey
            ? COLOR_ACTIVE[colorKey]
            : 'text-slate-400 hover:text-white hover:bg-white/10'
          }`}>
        <Icon size={19} strokeWidth={active ? 2.5 : 2}/>
        {badge && badge > 0
          ? <span className="absolute -top-1 -right-1 bg-rose-500 text-white text-[9px] font-bold
...
function ActionPanel({
  ...
  return (
    <div ref={ref}
      ...
      <div className="absolute bottom-[-7px] left-6
        ...
        {ACTION_GROUPS.map(group => (
          <div key={group.label}>
            ...
            <p className={`text-[9px] font-bold uppercase tracking-wider mb-1.5 px-1 flex items-center gap-1.5 ${COLOR_ICON[group.color]}`}>
              <span className={`w-1 h-3 rounded-full opacity-70 ${
                group.color === 'violet'  ? 'bg-violet-400' :
                group.color === 'teal'    ? 'bg-teal-400'   :
...
export const Taskbar: React.FC<TaskbarProps> = ({
  ...
  function handleAction(key: string) {
    switch (key) {
      ...
      // Báo cáo & Hồ sơ
      case 'weekly':   generateWeeklyReport(); break;
      case 'record':   setRecordType('Biên bản nghiệm thu'); setShowRecordForm(true); break;
      case 'diary':    setRecordType('Nhật ký công trình');  setShowRecordForm(true); break;

      // GEM & Vật tư
      case 'gem':      analyzeMaterialsWithGem(); break;
      case 'upload':   fileInputRef.current?.click(); break;
      case 'material': setRecordType('Phiếu yêu cầu vật tư'); setShowRecordForm(true); break;

      // Nhân lực & HSE
      case 'profile':  setShowProfileForm(true); break;
      case 'hse':      setShowHseForm(true); break;
      case 'hse-vio':  setRecordType('Biên bản vi phạm HSE'); setShowRecordForm(true); break;

      // Điều hướng nhanh
      case 'nav-progress':  setActiveTab('tasks'); setTimeout(() => onNavigateProject?.('progress'), 150); break;
      case 'nav-qaqc':      setActiveTab('tasks'); setTimeout(() => onNavigateProject?.('qa-qc'), 150); break;
      case 'nav-qs':        setActiveTab('tasks'); setTimeout(() => onNavigateProject?.('qs'), 150); break;
      case 'nav-equipment': setActiveTab('tasks'); setTimeout(() => onNavigateProject?.('equipment'), 150); break;
        ...
        <Tooltip label={isExpanded ? 'Thu gọn' : 'GEM & CLAUDE PM Pro'}>
          <button onClick={() => setIsExpanded(v => !v)}
            ...
            {isExpanded
              ...
              : <img src={`${window.location.origin}/icon_app_64.png`} alt="GEM PM" className="w-9 h-9 object-cover rounded-xl" onError={e=>{(e.target as HTMLImageElement).style.display='none'}}/>
            }
          </button>
        </Tooltip>
          ...
          <>
            ...
            {((pendingCount ?? 0) > 0 || isSyncing) && (
              <div className="shrink-0">
                <button onClick={onOpenQueuePanel}
...
export function UserMenuBar() {
  return (
    <>
      {/* Logo — top left */}
      <div className="fixed top-4 left-4 z-[9001] print:hidden flex items-center gap-2.5
