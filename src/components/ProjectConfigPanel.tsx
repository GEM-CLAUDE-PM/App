...
export async function loadProjectConfigAsync(projectId: string): Promise<ProjectConfig> {
  const data = await db.get<ProjectConfig>('project_config', projectId, EMPTY_CONFIG(projectId));
  return { ...EMPTY_CONFIG(projectId), ...data };
}
export async function saveProjectConfigAsync(cfg: ProjectConfig): Promise<void> {
  await db.set('project_config', cfg.projectId, cfg);
}
export async function loadProjectLogoAsync(projectId: string): Promise<string> {
  return db.get<string>('project_logo', projectId, '');
}
export async function saveProjectLogoAsync(projectId: string, base64: string): Promise<void> {
  await db.set('project_logo', projectId, base64);
}

/** @deprecated Sync shim — reads from localStorage cache written by db.ts */
export function loadProjectConfig(projectId: string): ProjectConfig {
  try {
    const raw = localStorage.getItem(`gem_db__project_config__${projectId}`);
...
export function saveProjectConfig(cfg: ProjectConfig): void {
  db.set('project_config', cfg.projectId, cfg);
}
/** @deprecated Sync shim */
export function loadProjectLogo(projectId: string): string {
  try { return localStorage.getItem(`gem_db__project_logo__${projectId}`) || ''; } catch { return ''; }
}
/** @deprecated Sync shim */
export function saveProjectLogo(projectId: string, base64: string): void {
  db.set('project_logo', projectId, base64);
}
